...
- 导出图片可以先做设计占位，后续前端用 canvas/html-to-image 实现。
...
实现不依赖底层：示例和本地最近输入可以存在浏览器内存或 localStorage；若隐私优先，可默认只保留本页面会话。
...
- 每站提供 UI 占位动作：替换、跳过、复制店名、在地图上定位。
- 不实现替换算法时，按钮可以进入“后续可接入”或“重新规划时排除该点”的轻提示。
- 移动端长按站点打开底部操作菜单。
...
实现不依赖底层：展开、复制、地图定位可前端完成；替换可先设计为未来能力。

### 2.7 反馈与可恢复

适用场景：请求失败、地图瓦片失败、无结果、用户误操作。

UI 方案：

- 生成中显示四步进度：解析意图、筛选地点、校验营业、生成路线。
- 无结果给出三个修正按钮：扩大半径、放宽时间、切换模式。
- 地图瓦片不可用时保留路线时间轴和静态路线示意。
...
- 结果带走：复制完整路线文本、复制 POI 名单、toast 反馈；导出图片保留为前端能力占位。
- 输入便利：示例 chip、使用本页面会话的上次目标、清空输入、空输入时高亮示例。
- 时间轴微交互：每站展示推荐理由、营业时间、评分、复制店名、地图定位、替换/跳过占位提示。
- 可恢复状态：生成中四步进度、无结果修正入口、地图资源不可用时保留时间轴。
...
- 实时拥挤/营业图层真实数据，需要真实平台 API 或稳定模拟数据源。
