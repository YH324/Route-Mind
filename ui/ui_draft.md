...
当前项目是一个成都武侯、锦江预研阶段的智能路线规划系统。核心能力不是普通 POI 搜索，而是把自然语言目标、用户模式、路网时间、营业时间、GT 评分、类型多样性和会话上下文组合成可执行路线。
...
默认展示春熙路示例和三种模式说明。地图显示默认中心点和 3km 半径。右侧显示“输入目标后生成路线”的轻提示，不放大段说明文案。
...
右侧给出具体原因：范围太小、当前时间未营业、单段移动限制过严、地图瓦片不可用。保留用户可操作的修正入口，如扩大半径、改时间、切换模式。
...
可打开 `ui/route_planner_concept.html` 查看高保真静态稿。它使用内置 mock 数据模拟当前 `/api/plan` 的 `variants`、`route`、`recommendations`、`constraints` 结构，不依赖后端、Leaflet 或 CDN。
