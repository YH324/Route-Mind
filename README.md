...
系统会根据意图输出单点候选、顺序候选路线或带时间轴的可执行路线，并为每个推荐点返回结构化推荐依据。
...
- [在线试用](http://124.223.28.124/)：已部署的 RouteMind Web 工作台。
...
- [默认值与语义兜底说明](docs/DEFAULTS_AND_GUARDRAILS.md)：说明哪些是必要工程边界，哪些不是固定推荐模板。
...
- **自然语言意图解析**：支持时间预算/出行方式/偏好类型/起始时间（如"下午四点"）；未明确时长时按单点推荐、两点组合、半日/一日、商务/夜间场景推断合理预算
...
- **结构化推荐依据**：每个 POI 返回 `recommendation_basis` 和面向用户的 `review_summary`，解释质量分、类型权重、评价热度估计、偏好/语义命中、营业状态、移动成本、路网可达性、类型一致性、数据驱动品牌识别、门店实体可信度和精选口碑摘要
...
# 本地 smoke test（不依赖外部模型时会走本地语义兜底）
...
候选质量治理会过滤停业/装修、门岗/入口、停车/快递/充电设施、民宿房源、普通销售经营点等低价值 POI；逛街+咖啡、喝酒+夜宵、午餐+咖啡、超市+小吃等组合需求会生成严格顺序约束，避免同大类泛化候选挤掉用户明确说出的类型。路网起点过严导致无起点时，路线构建会降级到直线距离起点选择，并在 `recommendation_basis.features.start_fallback` 中标注；若严格时间/步行约束下无法构成主路线，会返回带时间轴、移动距离和 `polyline` 的“顺序候选路线”，前端仍能在地图上画出线。

`feature_ranker_v1.5` / `route_planner_v3.9` 在 v1.4 的品牌识别基础上增加了更严格的服务范围、组合意图、动态时间预算和实体适配治理：

- `review_count_estimate` / `popularity_adjustment`：根据类型评论画像、商圈 POI 密度和本地聚合质量分估算评价样本量，让“名气/热度”与评分一起参与排序。
- `time_budget_source`：区分用户显式给出的时长和系统按意图推断的时长，避免所有请求默认 4 小时。
- `review_summary`：把内部评分/热度/场景信号转成用户可读的口碑摘要，供前端卡片和地图弹窗展示。
...
- `clarification_options`：当“随便安排一下”等短句无法确定类型时，服务返回澄清问题和可点击目标，支持多轮继续规划。
...
- 右栏：方案对比卡、只看差异、可滚动时间轴/单点推荐列表、精选口碑摘要、推荐理由、复制路线文本、复制 POI 名单、toast、无结果修正入口。
...
5. **LLM意图解析**：保留短查询确定性意图门控，使用 MiMo/MiniMax/GLM 处理模糊表达（如"想吃点辣的"→火锅/烧烤）
